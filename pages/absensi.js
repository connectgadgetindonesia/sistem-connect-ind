...
export default function AbsenTugasKaryawan() {
  ...
  async function handleSubmitAbsen(e) {
    ...
    if (ins.error) {
      return alert('Gagal absen: ' + ins.error.message)
    }
  ...
  const hadirNames = useMemo(
    () => (absenList || []).filter((a) => a.status === 'Hadir').map((a) => (a.nama || '').trim().toUpperCase()),
    [absenList]
  )
  ...
  async function loadTasks(tgl) {
    ...
    // tampilkan:
    // - manual: bebas
    // - otomatis: hanya canonical persis
    ...
    // ✅ HARI INI: tampil hanya TODO (DONE disembunyikan, tapi tetap ada di DB utk laporan)
    if (tgl === todayStr()) {
  ...
  async function ensureTasksIfNeeded(tgl) {
    ...
    try {
      ...
      // jangan setTasks manual di sini, selalu reload dari DB
    } finally {
  ...
  async function toggleTaskStatus(task) {
    ...
    const payload =
      next === 'done'
        ? { status: next, done_at: new Date().toISOString(), done_by: chosenAssignee, assignee: chosenAssignee }
    ...
    try {
      ...
      let upd = null
      if (task.id) {
        upd = await supabase.from('tugas_harian').update(payload).eq('id', task.id)
      }

      // 2) fallback: by task_date + title
      if (!upd || upd.error) {
        const tgl = task.task_date || selectedDate
        ...
        if (upd2.error) {
          console.error('toggleTaskStatus update failed:', upd?.error || upd2.error)
          return alert('Gagal menyimpan status tugas ke database. Cek RLS/permission tabel tugas_harian.')
        }
      }

      // ✅ reload dari DB biar refresh ga balik lagi
      await loadTasks(selectedDate)
    } catch (e) {
      console.error('toggleTaskStatus unexpected error:', e)
      alert('Terjadi error saat menyimpan tugas. Coba ulangi.')
    }
  }
      ...
      <div className="max-w-5xl mx-auto p-4">
        ...
        <div className="border rounded p-4">
          ...
          {tasks.length === 0 && isToday && absenList.some((a) => a.status === 'Hadir') && (
            <p className="text-sm text-gray-500 mt-2">Tidak ada tugas tersisa (semua sudah selesai).</p>
          )}
