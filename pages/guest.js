...
export default function GuestPage() {
  ...
  // ✅ supabase authed khusus guest (dari cookie token)
  const sb = useMemo(() => {
  ...
  async function boot() {
    ...
    if (!sb) {
      alert('Supabase env belum terdeteksi (NEXT_PUBLIC_SUPABASE_URL / ANON_KEY).')
      window.location.replace('/guest-login')
      return
    }
    ...
    await Promise.all([fetchPricelist(), fetchStokKategoriOptions(), fetchAksKategoriOptions()])
  }
  ...
  const plFiltered = useMemo(() => {
    ...
    let rows = (pricelist || []).filter((x) => {
      ...
      if (!okK) return false
      if (!q) return true
      return (x.nama_produk || '').toLowerCase().includes(q) || (x.kategori || '').toLowerCase().includes(q)
    })
  ...
  // ========= DOWNLOAD JPG (pakai kode kamu, tidak diubah) =========
  async function downloadJpgKategori() {
    try {
      ...
      const renderPageToBlob = async (items, pageIndex, totalPages) => {
        ...
        wrap.innerHTML = `
          <div style="height:${HEADER_H}px;padding:${PAD}px;color:#ffffff;
            background: linear-gradient(135deg, #0b1220 0%, #111827 55%, #0f172a 100%);position: relative;">
            <div style="font-size:18px; letter-spacing:2px; opacity:.9; font-weight:800;">CONNECT.IND • PRICELIST</div>
            <div style="margin-top:18px; font-size:72px; font-weight:900; line-height:1;">${String(kategori)}</div>
            <div style="margin-top:14px; font-size:26px; opacity:.9; font-weight:700;">Update: <b>${tanggal}</b></div>
          ...
          <div style="padding:${PAD}px;">
            <div style="border:1px solid #e5e7eb;border-radius:22px;overflow:hidden;
              box-shadow: 0 6px 20px rgba(15,23,42,0.06);background:#fff;">
              ...
              <div>
                ${items
                  ...
                  <div style="display:flex; border-top:1px solid #e5e7eb; background:#ffffff;">
                    <div style="flex:1; padding:20px 20px; font-size:22px; font-weight:900; letter-spacing:.2px;">${x.nama}</div>
                    <div style="width:320px; padding:20px 20px; font-size:26px; font-weight:900; text-align:right;">${x.harga}</div>
                  </div>
                `
                  )
                  .join('')}
          ...
          <div style="position:absolute;left:0; right:0; bottom:0;height:${FOOTER_H}px;
            padding: 0 ${PAD}px ${PAD}px ${PAD}px;display:flex;align-items:flex-end;justify-content:space-between;
            color:#64748b;font-size:20px;font-weight:700;background: transparent;">
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        if (chunks.length === 1) await saveBlob(filename, blob)
        else filesForZip.push({ name: filename, blob })
      }

      if (filesForZip.length > 0) await saveZip(`Pricelist-${safeKategori}.zip`, filesForZip)
    } catch (e) {
  ...
  async function fetchStok() {
    ...
    if (q) {
      const like = `%${q}%`
      query = query.or(`nama_produk.ilike.${like},warna.ilike.${like},garansi.ilike.${like},storage.ilike.${like}`)
    }
  ...
  return (
    <GuestLayout>
      ...
      <div className="bg-white border rounded-2xl shadow-sm p-4 md:p-5 mb-6">
        <div className="flex items-start justify-between gap-3 mb-3">
          ...
          <div className="flex gap-2">
            ...
            <button className="border px-4 py-2 rounded-lg bg-white hover:bg-slate-50" onClick={fetchPricelist} type="button">
              Refresh
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          <TabBtn active={!plKategori} onClick={() => (setPlKategori(''), setPlPage(1))}>
            Semua
          </TabBtn>
          {plKategoriOptions.map((k) => (
            <TabBtn key={k} active={plKategori === k} onClick={() => (setPlKategori(k), setPlPage(1))}>
              {k}
            </TabBtn>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 mb-4">
          ...
          <div className="md:col-span-3">
            <select className="border p-2.5 rounded-lg w-full bg-white" value={plSort} onChange={(e) => (setPlSort(e.target.value), setPlPage(1))}>
            ...
            <tbody>
              {plRows.map((x) => (
                <tr key={x.id} className="border-t">
                  <td className="px-4 py-3 font-semibold">{String(x.nama_produk || '').toUpperCase()}</td>
                  <td className="px-4 py-3 text-right">{formatRp(x.harga_tokped)}</td>
                  <td className="px-4 py-3 text-right">{formatRp(x.harga_shopee)}</td>
                  <td className="px-4 py-3 text-right font-bold">{formatRp(x.harga_offline)}</td>
                </tr>
              ))}
              {plRows.length === 0 && (
                <tr>
        ...
        <div className="flex items-center justify-between mt-3">
          ...
          <div className="flex gap-2">
            <button className="border px-4 py-2 rounded-lg bg-white hover:bg-slate-50 disabled:opacity-50" disabled={plPage <= 1} onClick={() => setPlPage((p) => Math.max(1, p - 1))} type="button">
              Prev
            </button>
            <button className="border px-4 py-2 rounded-lg bg-white hover:bg-slate-50 disabled:opacity-50" disabled={!plHasNext} onClick={() => setPlPage((p) => p + 1)} type="button">
              Next
            </button>
          </div>
        </div>
      ...
      <div className="bg-white border rounded-2xl shadow-sm p-4 md:p-5 mb-6">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          <TabBtn active={!stokKategori} onClick={() => (setStokKategori(''), setStokPage(1))}>
            Semua
          </TabBtn>
          {stokKategoriOptions.map((k) => (
            <TabBtn key={k} active={stokKategori === k} onClick={() => (setStokKategori(k), setStokPage(1))}>
              {k}
            </TabBtn>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 mb-4">
          <div className="md:col-span-9">
            <input className="border p-2.5 rounded-lg w-full" placeholder="Cari nama / warna / garansi / storage..." value={stokSearch} onChange={(e) => setStokSearch(e.target.value)} />
          </div>
          <div className="md:col-span-3">
            <button className="border px-4 py-2 rounded-lg bg-white hover:bg-slate-50 w-full" onClick={() => (setStokPage(1), fetchStok())} type="button">
              Terapkan
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mt-3">
          ...
          <div className="flex gap-2">
            <button className="border px-4 py-2 rounded-lg bg-white hover:bg-slate-50 disabled:opacity-50" disabled={stokPage <= 1} onClick={() => setStokPage((p) => Math.max(1, p - 1))} type="button">
              Prev
            </button>
            <button className="border px-4 py-2 rounded-lg bg-white hover:bg-slate-50 disabled:opacity-50" disabled={!stokHasNext} onClick={() => setStokPage((p) => p + 1)} type="button">
              Next
            </button>
          </div>
        </div>
      ...
      <div className="bg-white border rounded-2xl shadow-sm p-4 md:p-5">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          <TabBtn active={!aksKategori} onClick={() => (setAksKategori(''), setAksPage(1))}>
            Semua
          </TabBtn>
          {aksKategoriOptions.map((k) => (
            <TabBtn key={k} active={aksKategori === k} onClick={() => (setAksKategori(k), setAksPage(1))}>
              {k}
            </TabBtn>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 mb-4">
          <div className="md:col-span-9">
            <input className="border p-2.5 rounded-lg w-full" placeholder="Cari nama / warna..." value={aksSearch} onChange={(e) => setAksSearch(e.target.value)} />
          </div>
          <div className="md:col-span-3">
            <button className="border px-4 py-2 rounded-lg bg-white hover:bg-slate-50 w-full" onClick={() => (setAksPage(1), fetchAks())} type="button">
              Terapkan
            </button>
          </div>
        </div>
            ...
            <tbody>
              {(aks || []).map((x) => (
                <tr key={x.id} className="border-t">
                  <td className="px-4 py-3 font-semibold">{String(x.nama_produk || '').toUpperCase()}</td>
                  <td className="px-4 py-3">{x.warna || '-'}</td>
                  <td className="px-4 py-3 text-right font-bold">{x.stok ?? 0}</td>
                </tr>
              ))}
              {aks.length === 0 && (
                <tr>
        ...
        <div className="flex items-center justify-between mt-3">
          ...
          <div className="flex gap-2">
            <button className="border px-4 py-2 rounded-lg bg-white hover:bg-slate-50 disabled:opacity-50" disabled={aksPage <= 1} onClick={() => setAksPage((p) => Math.max(1, p - 1))} type="button">
              Prev
            </button>
            <button className="border px-4 py-2 rounded-lg bg-white hover:bg-slate-50 disabled:opacity-50" disabled={!aksHasNext} onClick={() => setAksPage((p) => p + 1)} type="button">
              Next
            </button>
          </div>
        </div>
...
async function saveZip(zipFilename, files) {
