...
export default function Penjualan() {
  ...
  function tambahBonusKeList() {
    ...
    setBonusBaru({ sn_sku: '', nama_produk: '', warna: '', harga_modal: '', garansi: '', storage: '' })
  }
  ...
  async function handleSubmit(e) {
    ...
    const semuaProduk = [
      ...produkList.map(p => ({ ...p, harga_jual: parseInt(p.harga_jual), is_bonus: false })),
      ...bonusList.map(b => ({ ...b, harga_jual: 0, is_bonus: true }))
    ]

    for (const produk of semuaProduk) {
      const harga_modal = parseInt(produk.harga_modal)
      const laba = produk.harga_jual - harga_modal
      ...
      <div className="p-4">
        ...
        <form onSubmit={handleSubmit} className="grid gap-4 mb-6">
          ...
          <div className="border p-4 rounded bg-yellow-50">
            ...
            <Select className="text-sm mb-2" options={options} placeholder="Cari SN / SKU Bonus" value={options.find((opt) => opt.value === bonusBaru.sn_sku) || null} onChange={(selected) => setBonusBaru({ ...bonusBaru, sn_sku: selected?.value || '' })} isClearable />
            <button type="button" onClick={tambahBonusKeList} className="bg-yellow-600 text-white px-4 py-1 rounded">Tambah Bonus</button>
          </div>
          ...
          {bonusList.length > 0 && (
            <div className="border rounded p-4 bg-white">
              ...
              <ul className="text-sm list-disc ml-4">
                {bonusList.map((b, i) => (
                  <li key={i}>{b.nama_produk} ({b.sn_sku}) - BONUS</li>
                ))}
              </ul>
            </div>
          )}
