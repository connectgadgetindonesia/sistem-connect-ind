 ...
 * - write: update ONLY harga_tokped (sesuai DB)
 */
...
export default function PricelistPage() {
  ...
  // ✅ SEARCH + SORT (dipakai untuk tabel & download)
  const filteredRows = useMemo(() => {
    const q = String(search || '').trim().toLowerCase()
    let arr = rows

    if (q) {
      arr = arr.filter((r) => String(r.nama_produk || '').toLowerCase().includes(q))
    }

    const copy = [...arr]
    const byName = (a, b) =>
      String(a.nama_produk || '').localeCompare(String(b.nama_produk || ''), 'id', { sensitivity: 'base' })
    const byPrice = (a, b) => toNumber(a.harga_offline) - toNumber(b.harga_offline)
    ...
    <Layout>
      <div style={{ padding: 24 }}>
        ...
        <div style={card}>
          ...
          {/* ===== AREA KHUSUS UNTUK DOWNLOAD JPG (hidden offscreen) ===== */}
          <div
            ref={printRef}
            style={{
              position: 'fixed',
              ...
              <div style={{ marginTop: 14 }}>
                ...
                <div style={fieldRow}>
                  ...
                  <select
                    ...
                    onChange={(e) => setBulkForm((p) => ({ ...p, kategori: normalizeKategoriLabel(e.target.value) }))}
                  >
                    {kategoriList.map((k) => (
                      <option key={k} value={k}>
                        {k}
